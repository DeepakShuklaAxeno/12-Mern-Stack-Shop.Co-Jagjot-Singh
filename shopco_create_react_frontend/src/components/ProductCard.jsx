  ...
  return (
    <article className="min-w-0">
      <Link className="block overflow-hidden rounded-2xl bg-[#f0eeed]" to={item._id ? `/products/${item._id}` : "/products"}>
        {image ? <img alt={item.name} className="aspect-square w-full h-full  object-cover transition-transform duration-300 hover:scale-105" src={getImageUrl(image)} /> : <div className="aspect-square h-full w-full bg-[#f0eeed]" />}
      </Link>
